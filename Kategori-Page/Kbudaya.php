...
");
$stmt->execute();
$konten2ArticlesBudaya = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Query untuk "Pilihan Untukmu" Budaya
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Budaya' 
...
// Query untuk "Editor Pick" Budaya
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Budaya' 
...
");
$stmt->execute();
$editorPickBudayaArticles = $stmt->fetchAll(PDO::FETCH_ASSOC);

$mainArticle = !empty($editorPickBudayaArticles) ? array_shift($editorPickBudayaArticles) : null;

// Query untuk "Sorotan Class" Budaya
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Budaya' 
...
<head>
    ...
    <title>Kategori - Budaya</title>
    <link rel="stylesheet" href="Kbudaya.css">
    ...
    <main>
        ...
        <div class="container">
            <div class="konten1-flex">
                <?php if (!empty($konten1ArticlesBudaya)): ?>
                    <?php foreach ($konten1ArticlesBudaya as $index => $article): ?>
                        <?php if ($index === 0): ?>
            ...
            <?php foreach ($konten2ArticlesBudaya as $article): ?>
                <div class="panel">
        ...
        <div class="konten-editor-pick-budaya">
            <div class="kontainer">
                <div class="gambar-kiri">
                    <?php foreach (array_slice($editorPickBudayaArticles, 0, 2) as $article): ?>
                        <div class="card">
                ...
                <div class="gambar-kanan">
                    <?php foreach (array_slice($editorPickBudayaArticles, 2, 2) as $article): ?>
                        <div class="card">
                            <img src="../assets/<?= htmlspecialchars($article["gambar"]) ?>" alt="<?= htmlspecialchars($article["judul"]) ?>" class="gambar-sisi">
                            <div class="card-konten">
                                <h3>
