...
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis
    ...
    WHERE articles.page_target = 'Olahraga' 
      AND articles.section = 'konten-1' 
      AND articles.is_verified = 1
    ORDER BY articles.tanggal DESC
");
$stmt->execute();
$konten1ArticlesOlahraga = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Query untuk konten-2 Olahraga
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Olahraga' 
...
");
$stmt->execute();
$konten2ArticlesOlahraga = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Query untuk "Pilihan Untukmu" Olahraga
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Olahraga' 
...
// Query untuk "Editor Pick" Olahraga
$stmt = $conn->prepare(" SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Olahraga' 
...
");
$stmt->execute();
$editorPickOlahragaArticles = $stmt->fetchAll(PDO::FETCH_ASSOC);

$mainArticle = !empty($editorPickOlahragaArticles) ? array_shift($editorPickOlahragaArticles) : null;

// Query untuk "Sorotan Class" Olahraga
$stmt = $conn->prepare("SELECT articles.*, users.username AS penulis 
    FROM articles 
    JOIN users ON articles.author_id = users.id 
    WHERE articles.page_target = 'Olahraga' 
...
<head>
    ...
    <title>Kategori - Olahraga</title>
    <link rel="stylesheet" href="Kolahraga.css">
...
<body>
    ...
    <main>
        ...
        <div class="konten1-flex">
            <?php if (!empty($konten1ArticlesOlahraga)): ?>
                <?php foreach ($konten1ArticlesOlahraga as $index => $article): ?>
                    <?php if ($index === 0): ?>
        ...
        <div class="konten-editor-pick-Olahraga">
            <div class="kontainer">
                <div class="gambar-kiri">
                    <?php foreach (array_slice($editorPickOlahragaArticles, 0, 2) as $article): ?>
                        <div class="card">
                ...
                <div class="gambar-kanan">
                    <?php foreach (array_slice($editorPickOlahragaArticles, 2, 2) as $article): ?>
                        <div class="card">
                            <img src="../assets/<?= htmlspecialchars($article["gambar"]) ?>" alt="<?= htmlspecialchars($article["judul"]) ?>" class="gambar-sisi">
                            <div class="card-konten">
                                <h3>
